RSpec.describe 'displays ask_for_extension token content', app: :api_particulier do
  subject(:got_to_ask_for_extension_token_page) do
    visit api_particulier_token_ask_for_extension_path(id: token.id)
  end
  ...
  before do
    ...
    got_to_ask_for_extension_token_page
  end
  ...
  describe 'when the token cannot be ask_for_extension' do
  ...
  describe 'when the token can be ask_for_extension' do
    let!(:token) do
      create(:token, authorization_request:, exp: 83.days.from_now.to_i)
    end

    it 'displays the page' do
      expect(page).to have_current_path(api_particulier_token_ask_for_extension_path(id: token.id), ignore_query: true)
    end
  end
end
