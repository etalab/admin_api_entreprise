...
RSpec.describe 'token details page', app: :api_entreprise do
  let(:demandeur) { create(:user, :with_token) }
  let(:authorization_request) { create(:authorization_request, :with_demandeur, :with_contact_technique, demandeur:) }
  ...
  shared_examples 'it can see/share tokens' do
  ...
  context 'when user is demandeur' do
    let(:user) { demandeur }

    it_behaves_like 'it can see/share tokens'
  ...
  context 'when user is contact' do
    let(:user) { authorization_request.contact_technique }

    it_behaves_like 'it can see/share tokens'

    it 'doesnt have a link for token renewal' do
      expect(page).not_to have_link(href: token_renew_path(token))
    end

    it 'doesnt have a link to authorization request' do
